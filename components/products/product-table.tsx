          ...
          {products.length === 0 ? (
            ...
            products.map((product) => (
              <tr key={product.id} className="border-b border-border hover:bg-card/50 transition-colors">
                <td className="p-3 md:p-4 font-medium text-foreground text-sm md:text-base">{product.name}</td>
                <td className="p-3 md:p-4 text-muted-foreground text-xs md:text-sm">{product.sku}</td>
                <td className="p-3 md:p-4 text-muted-foreground text-xs md:text-sm">{product.category}</td>
                <td className="p-3 md:p-4 font-medium text-foreground text-right text-sm md:text-base">${product.price.toFixed(2)}</td>
                <td className="p-3 md:p-4 text-muted-foreground text-right text-sm md:text-base">${product.cost.toFixed(2)}</td>
