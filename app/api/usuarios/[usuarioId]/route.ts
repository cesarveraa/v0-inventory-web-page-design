// app/api/usuarios/[usuarioId]/route.ts
...
export async function PATCH(
  ...
  const { usuarioId } = await context.params   // 👈 aquí está la clave
  const backendPath = `/usuarios/${usuarioId}`
...
export async function DELETE(
  ...
  const { usuarioId } = await context.params   // 👈 igual aquí
  const backendPath = `/usuarios/${usuarioId}`
